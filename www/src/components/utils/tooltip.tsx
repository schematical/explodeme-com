    ...
    >
      ...
      <div className={`z-10 absolute ${positionOuterClasses(position)}`}>
        <Transition
          show={tooltipOpen}
          as="div"
          className={`min-w-[12rem] bg-slate-800 p-2 rounded overflow-hidden transform transition duration-200 ease-out data-[enter]:data-[closed]:-translate-y-2 data-[closed]:opacity-0 ${positionInnerClasses(position)}`}
        >
          {children}
        </Transition>
      </div>
